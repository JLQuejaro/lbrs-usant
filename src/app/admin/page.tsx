             ...
             {['Users', 'Account Requests', 'Analytics', 'Audit Logs', 'System'].map((tab) => {
               ...
               return (
                 ...
                 >
                   {tab}
                   {pendingCount > 0 && (
                     <span className="bg-white text-usant-red text-[10px] font-black px-1.5 py-0.5 rounded-full">
                       {pendingCount}
                     </span>
                   )}
                 </button>
              ...
              <div className="p-6 border-b border-gray-100 flex items-center justify-between sticky top-0 bg-white rounded-t-2xl">
                <div className="flex items-center gap-3">
                  <div className={`p-2 rounded-lg ${
                    selectedRequest.requestedRole === 'faculty' ? 'bg-blue-50 text-blue-600' : 'bg-orange-50 text-orange-600'
                  }`}>
                    <UserCheck size={24} />
                  </div>
                  <div>
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="flex items-center justify-center">
                  <span className={`px-4 py-2 rounded-full text-sm font-bold uppercase flex items-center gap-2 ${
                    ...
                    {selectedRequest.status === 'pending' && <Hourglass size={16} />}
                    {selectedRequest.status === 'approved' && <CheckCircle size={16} />}
                    {selectedRequest.status === 'rejected' && <XCircle size={16} />}
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="p-4 bg-gray-50 rounded-xl">
                    ...
                    <p className="font-bold text-gray-900 mt-1">{selectedRequest.email}</p>
                  </div>
                  <div className="p-4 bg-gray-50 rounded-xl">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">Requested Role</label>
                    <p className="font-bold text-gray-900 mt-1 capitalize">{selectedRequest.requestedRole}</p>
                  </div>
                  <div className="p-4 bg-gray-50 rounded-xl">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">User Type</label>
                    <p className="font-bold text-gray-900 mt-1">{selectedRequest.userType}</p>
                  </div>
                  <div className="p-4 bg-gray-50 rounded-xl">
                    <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider">
                      {selectedRequest.requestedRole === 'faculty' ? 'Department' : 'Course'}
                    </label>
                    <p className="font-bold text-gray-900 mt-1">{selectedRequest.course || selectedRequest.department || 'N/A'}</p>
                  </div>
                  <div className="p-4 bg-gray-50 rounded-xl">
                ...
                {selectedRequest.idDocument && (
                  <div className="p-4 bg-blue-50 rounded-xl border border-blue-100">
                    ...
                    <div className="flex items-center justify-between mt-2">
                      <p className="font-medium text-blue-900">{selectedRequest.idDocument}</p>
                  ...
                  <div className="p-4 bg-gray-50 rounded-xl border border-gray-200">
                    ...
                    <div className="mt-2 space-y-1">
                      <p className="text-sm text-gray-700">
                        <span className="font-bold">Reviewed by:</span> {selectedRequest.reviewedBy}
                      </p>
                      <p className="text-sm text-gray-700">
                        <span className="font-bold">Reviewed at:</span> {new Date(selectedRequest.reviewedAt!).toLocaleString()}
                      </p>
